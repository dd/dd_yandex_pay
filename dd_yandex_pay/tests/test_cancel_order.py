...
def test_request_data(mocked_uuid4, mocked_request):
    ...
    yp_client.cancel_order(order_id="test_123456", reason="Cancel for test")

    mocked_request.assert_called_once_with(
        "POST",
        "http://127.0.0.1/v1/orders/test_123456/cancel",
...
def test_usage_yandexpayclient_get_url(mocked_request, mocked_get_url):
    ...
    mocked_get_url.assert_called_once_with("v1/orders/test_123456/cancel")
...
def test_returned_data(mocked_request):
    ...
    yp_client = YandexPayClient("api-key", base_url="http://127.0.0.1/")
    response = yp_client.cancel_order(order_id="test_123456", reason="Cancel for test")
...
def test_character_conversion(mocked_request):
    ...
    yp_client = YandexPayClient("api-key", base_url="http://127.0.0.1/")
    yp_client.cancel_order(order_id="test #1/1", reason="Cancel for test")

    assert mocked_request.call_args.args[1] == "http://127.0.0.1/v1/orders/test%20%231%2F1/cancel"
