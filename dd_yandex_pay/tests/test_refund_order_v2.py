...
def test_request_data(mocked_uuid4, mocked_request):
    ...
    mocked_request.assert_called_once_with(
        "POST",
        "http://127.0.0.1/v2/orders/test_123456/refund",
        json={"refundAmount": 123.45},
        timeout=(3, 10),
        headers={
...
def test_all_data_params(mocked_request):
    ...
    yp_client.refund_order_v2(
        ...
        targetCart=[{"productId": "product 1"}],
        targetShipping={"amount": 123.45},
    )
...
def test_usage_yandexpayclient_get_url(mocked_request, mocked_get_url):
    ...
    mocked_get_url.assert_called_once_with("v2/orders/test_123456/refund")
...
def test_returned_data(mocked_request):
    ...
    yp_client = YandexPayClient("api-key", base_url="http://127.0.0.1/")
    response = yp_client.refund_order_v2(
        order_id="test_123456",
        refundAmount=123.45,
    )
...
def test_character_conversion(mocked_request):
    ...
    assert mocked_request.call_args.args[1] == "http://127.0.0.1/v2/orders/test%20%231%2F1/refund"
