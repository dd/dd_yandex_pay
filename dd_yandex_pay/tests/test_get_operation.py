...
def test_request_data(mocked_uuid4, mocked_request):
    ...
    yp_client.get_operation(external_operation_id="test_123456")

    mocked_request.assert_called_once_with(
        "GET",
        "http://127.0.0.1/v1/operations/test_123456",
        timeout=(3, 10),
        headers={
...
def test_usage_yandexpayclient_get_url(mocked_request, mocked_get_url):
    ...
    mocked_get_url.assert_called_once_with("v1/operations/test_123456")
...
def test_passing_additional_parameters(mocked_request, mocked_uuid4):
    ...
    yp_client = YandexPayClient("api-key", base_url="http://127.0.0.1/")
    yp_client.get_operation(
        external_operation_id="test_123456",
...
def test_returned_data(mocked_request):
    ...
    yp_client = YandexPayClient("api-key", base_url="http://127.0.0.1/")
    response = yp_client.get_operation(external_operation_id="test_123456")
...
def test_character_conversion(mocked_request):
    ...
    yp_client = YandexPayClient("api-key", base_url="http://127.0.0.1/")
    yp_client.get_operation(external_operation_id="test #1/1")

    assert mocked_request.call_args.args[1] == "http://127.0.0.1/v1/operations/test%20%231%2F1"
