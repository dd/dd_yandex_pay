...
logger = logging.getLogger("dd_yandex_pay")
...
def generate_code_section():
    ...
    """
    if os.environ.get("DDYP_MKDOCS_CODE_PARCE_ENABLED", "t").lower() not in TRUE_CHOICES:
        return

    # parse and register
    source_path = Path(os.environ.get("DDYP_MKDOCS_CODE_PARCE_SOURCE_PATH", "dd_yandex_pay"))
    doc_path = Path(os.environ.get("DDYP_MKDOCS_CODE_PARCE_DOCS_PATH", "reference"))
    ...
    if os.environ.get("DDYP_MKDOCS_CODE_PARCE_SHOW_NAV", "f").lower() in TRUE_CHOICES:
